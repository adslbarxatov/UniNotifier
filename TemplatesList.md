```
10.05.2021
Стена ВКонтакте				https://vk.com/{ID_ИЛИ_НАЗВАНИЕ_ГРУППЫ}						class="pi_text					</div>		2
Число подписчиков Instagram	https://www.instagram.com/{USERNAME}						•								</title>	1
Число подписчиков ВК		https://vk.com/{ID_ИЛИ_НАЗВАНИЕ_ГРУППЫ}						act=members">					</em>		1
КоммерсантЪ					https://www.kommersant.ru									from=hotnews">					</h3>		1
Российская газета			https://rg.ru												html" class="b-link">			</div>		1
Весьма						http://vesma.today/news										class="color-text">				</div>		1
Погода на Mail.ru			https://pogoda.mail.ru/prognoz/{ГОРОД}						property="og:title" content="	" />		1
Погода на YR				https://retro.yr.no/place/Russland/{CITY}/{STATION}			"og:description" content="		" />		1
Yahoo finance				https://finance.yahoo.com/quote/{STOCK}/news?p={STOCK}		StretchedBox" data-reactid=		</div>		1

***

Tested sites:

codeproject.com, articles page	(works)
kommersant.ru, main page		(works)
moddb.com, mod comments			(works)
moddb.com, latest mods			(fails to find needed section)
rbc.ru, main page				(fails with error 404)
rg.ru, main page				(works)
vesma.today, news section		(works)
vk.com, community wall			(works)
youtube.com, Youtube studio		(fails to parse page containment)
finance.yahoo.com, stock news	(works)
yr.no, old weather page			(works)
```
